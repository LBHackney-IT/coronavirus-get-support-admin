...
module.exports = {
  ...
  all_callbacks_get: async (req, res, next) => {
    try {
      await HelpRequestsService.getAllCallbacks({ master: true }).then(
        (result) => {
  ...
  help_request_create_post: async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        ...
        await HelpRequestsService.createHelpRequest(query, userName).then(
          (result) => {
            res.render("help-requests/help-request-create-success.njk", {
              query: result,
            });
          }
        );
